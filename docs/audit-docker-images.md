...
```mermaid
flowchart TD
    QEMU["Set up QEMU (existing)"] --> BUILDX["Set up Docker Buildx (existing)"]
    BUILDX --> BUILD["Build Docker image (existing)<br/>builds the image locally (push: false, load: true)<br/>image is now in the runner's Docker daemon"]
    BUILD --> SCAN["Scan Docker image<br/>scans the local image by name<br/>outputs a table to trivy-results.txt<br/>fails the pipeline on HIGH/CRITICAL (exit-code: 1)"]
    SCAN --> UPLOAD["Upload scan results<br/>appends trivy-results.txt to the job summary (if: !cancelled())"]
    UPLOAD --> LOGIN["Login to Docker Hub (existing)<br/>runs after the scan, right before the push"]
    LOGIN --> PUSH["Push to Docker Hub (existing)<br/>re-invokes docker/build-push-action with push: true"]
```
